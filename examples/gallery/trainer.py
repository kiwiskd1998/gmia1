...
from examples.gallery.callbacks import CallbackList, Scheduler, Optimizer
from torch.utils.data import DataLoader, Dataset
from examples.gallery.backend.backend import backend
import examples.gallery.utils.seed as aa
from examples.gallery.Graph.base_graph import BaseGraph
from examples.gallery.Graph import functional as gf
from examples.gallery.Graph.progbar import Progbar
...
class Trainer:
    ...
    def setup_graph(self, graph, graph_transform=None, device=None, **kwargs):
        ...
        此方法用于处理您的输入，它只接受您定义的方法“data_step”中的关键字参数。此方法将处理输入，并将它们转换为张量
    ...
    def fit(self, train_data, val_data=None, epochs=100, callbacks=None, verbose=None):
        ...
        try:
            for epoch in range(epochs):
                ...
                train_logs = self.train_step(train_data)  #用到latgcn中的train_step
    ...
    def graph(self, graph):
        assert graph is None or isinstance(graph, BaseGraph)
