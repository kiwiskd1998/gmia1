...
class PtbDataset:
	...
	def __init__(self, root, name, attack_method='mettack'):
		...
		self.url = 'https://raw.githubusercontent.com/ChandlerBang/pytorch-gnn-meta-attack/master/pre-attacked/{}_{}_0.05.npz'.format(
			self.name, self.attack_method)
	...
	def load_data(self):
		...
		warnings.warn(
			'''the adjacency matrix is perturbed, using the data splits under seed 15(default seed for deeprobust.graph.data.Dataset), so if you are going to verify the attacking performance, you should use the same data splits''')
		return adj
...
class PrePtbDataset:
	...
	def load_data(self):
		...
		if self.attack_method == 'meta':
			warnings.warn(
				"The pre-attacked graph is perturbed under the data splits provided by ProGNN. So if you are going to verify the attacking performance, you should use the same data splits  (setting='prognn').")
			adj = sp.load_npz(self.data_filename)
	...
	def get_target_nodes(self):
		...
		url = 'https://raw.githubusercontent.com/ChandlerBang/Pro-GNN/master/nettack/{}_nettacked_nodes.json'.format(
			self.name)
